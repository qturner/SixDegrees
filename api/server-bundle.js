...
async function withRetry(operation, maxRetries = 3) {
  ...
  const actualMaxRetries = isProd2 ? Math.min(maxRetries, 3) : maxRetries;
  for (let attempt = 1; attempt <= actualMaxRetries; attempt++) {
    try {
      ...
      if (isRetryableError) {
        const baseDelay = isProd2 ? 500 : 2e3;
        const delay = Math.min(baseDelay * Math.pow(1.5, attempt - 1), 1e4);
...
var isProd, internalPool, internalDb, getPool, getDb, pool, db;
var init_db = __esm({
  async "server/db.ts"() {
    "use strict";
    init_schema();
    ...
    getDb = () => {
      if (!internalDb) {
        internalDb = drizzle({ client: getPool(), schema: schema_exports });
      }
      return internalDb;
    };
    pool = new Proxy({}, {
...
// server/routes.ts
function getESTDateString(date = /* @__PURE__ */ new Date()) {
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/health", async (_req, res) => {
    try {
      const { checkDatabaseHealth: checkDatabaseHealth2 } = await init_db().then(() => db_exports);
      const dbHealthy = await checkDatabaseHealth2();
  ...
  cron.schedule("0 0 * * *", async () => {
    try {
      ...
      while (retryCount < maxRetries && !promotionSuccessful) {
        try {
          storage2 = (await init_storage().then(() => storage_exports)).storage;
...
function getESTDateString2(date = /* @__PURE__ */ new Date()) {
  const formatter = new Intl.DateTimeFormat("en-CA", {
    timeZone: "America/New_York",
    year: "numeric",
    month: "2-digit",
